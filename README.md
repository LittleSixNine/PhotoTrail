...
日常生活中有很多通过地图选点，为相机照片和胶片扫描文件补上拍摄位置的需求。但部分工具未正确衔接国内地图坐标体系，容易出现显示偏移，也很难标注正确的位置。

为了减少国内地图坐标体系混用造成的偏移，我在 GeoTag 原版基础上开发了 GeoTag CN，加入高德地图、坐标转换、地点收藏和中文界面，让日常定位更方便。
...
- **简体中文界面**：顶部切换列表与详情；列表用于筛选和批量操作，详情将照片预览、定位工具与大地图组合展示。
...
1. 首次启动按引导选择是否备份照片，并设置默认地图；高德密钥可以稍后填写。打开照片，或把文件拖入窗口。
2. 在详情页的地图设置菜单中，通过“高德 API 设置”填写自己的 **Web 端 JS API Key** 和 **securityJsCode**。未填写时，地图区域显示设置说明。
