...
struct ImageTableView: View {
        ...
        VStack(spacing: 0) {
            ...
            HStack {
                if !workspace.listMatchResults.isEmpty {
                    Toggle(L10n.text("仅看上次匹配未成功的照片"), isOn: $unmatchedOnly).toggleStyle(.checkbox)
    ...
    private var photoTable: some View {
        ...
        return Table(of: ImageData.self, selection: $selection, sortOrder: $sortOrder) {
            ...
            TableColumn(L10n.text("文件名"), value: \.name) { image in
        ...
        .alert(L10n.text("清除照片定位？"), isPresented: Binding(get: { !pendingClear.isEmpty }, set: {
            if !$0 { pendingClear = [] }
        })) {
            Button(L10n.text("取消"), role: .cancel) { pendingClear = [] }
            Button(L10n.text("清除定位"), role: .destructive) {
                store.send(.selectionChanged(pendingClear), undoable: false)
                store.send(.deleteRequest, description: L10n.text("清除照片定位"))
                pendingClear = []
            }
        } message: {
            Text(L10n.text("保留照片和列表项目，清除这些照片的坐标及相关地点信息。配对 RAW 同步修改，点击保存后才写入原文件；可撤销。"))
        }
        .overlay {
