...
地图左上角可切换标准/卫星视图。地图启动默认请求一次系统定位以确定初始视野；右下角定位按钮可再次请求定位并移动地图。两者均不修改照片。
...
- 两个地图共用标准/卫星滑道、指南针及缩放/本机定位控件。搜索使用 regular 雾面玻璃，收起为 44 点圆形放大镜，展开为同高胶囊，遵循减少动态效果设置。入口距地图底边 52 点，避让比例尺；结果面板显示在搜索框上方。
